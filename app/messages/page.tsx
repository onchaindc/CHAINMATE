...
export default function MessagesPage() {
  ...
  const conversations = useMemo<Conversation[]>(() => {
    ...
    for (const m of inbox) {
      ...
      // The peer's name is exactly what the server recorded — verbatim, never
      // a label the app derived. The "You" guard keeps legacy envelopes from
      // labeling the peer with the reader; an unnamed peer shows the
      // blank-name placeholder.
      // Every candidate name goes through guestDisplayName: real usernames
      // verbatim, machine-minted guest artifacts as the plain word "Guest".
      const peerName =
    ...
    openPeer({
      ...
      username:
        hit.counterpartName && hit.counterpartName !== "You"
          ? guestDisplayName(hit.counterpartName)
          : hit.fromPlayerId === deepLinkPeer
            ? guestDisplayName(hit.fromName)
            : "",
      is_guest: false,
      rating: 0,
      country: null,
