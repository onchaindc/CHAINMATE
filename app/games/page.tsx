...
const EMPTY_PLAYERS: Record<string, PlayerInfo> = {};
...
function GamesContent() {
  ...
  // Cached read: the games list (with deltas and the name map folded in) is
  // keyed per player, so revisiting this section renders the last-known list
  // instantly and revalidates in the background.
  const { data, error } = useCachedRead<{ games: GameState[]; deltas: Map<string, number>; players: Record<string, PlayerInfo> }>(
    `games:mine:${identity.playerId}`,
    async () => {
      ...
      return {
        games: mergeGamesById([...remote.games, ...localGames]),
    ...
    {
      // Wait for the real identity, like the profile page does — fetching on
      // the empty interim id returns an empty record.
      enabled: identity.status !== "loading" && Boolean(identity.playerId),
    },
  );
