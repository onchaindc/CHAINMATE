 ...
 * (Games / Profile). Explains that progress lives on this device and offers
 * the one-time upgrade — never an annoying modal, never on the board.
 */
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-3 gap-y-2 rounded-lg border border-primary/25 bg-primary/5 px-4 py-3">
      ...
      <p className="min-w-0 flex-1 text-xs leading-relaxed text-foreground/80">
        Playing as <span className="font-mono text-primary">{identity.username}</span> — your
        rating, games and achievements are saved on this device.
      </p>
      <Link
        href="/auth?upgrade=1"
        className="inline-flex items-center gap-1 text-xs font-semibold text-primary underline-offset-2 hover:underline"
      >
        Create account to keep them <ArrowRight className="h-3 w-3" aria-hidden />
      </Link>
    </div>
  );
}
