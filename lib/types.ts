...
export interface GameState {
  ...
  /**
   * Unix ms each side was actually PRESENT at the board (joined the game
   * page for a tournament match). Both players must have arrived before the
   * clock starts running — a player opening the game page late must never
   * find their clock already drained. Set by the server's arrive action.
   */
  arrivedAt?: { white?: number; black?: number };
  /** Unix ms both players were present and the clock began ticking. */
  clockStartedAt?: number;
  /** Set on a match generated by the tournament engine (the owning event). */
  tournamentId?: string;
  /**
...
export interface GameStore {
  ...
  /**
   * Check in as present for a tournament match. Idempotent; harmless in a
   * casual game. Once both sides have arrived the clock is started and the
   * returned state carries clockStartedAt.
   */
  arrive(id: string): Promise<GameState>;
